...
const spawnWave = () => {
    ...
    const spawnEnemy = () => {
        totalEnemyCount += 1;
        const spawnPt = document
            .getElementById("startPt")
        ...
        const enemyMovement = () => {
            const endPt = document
                .getElementById("endPt")
                .getBoundingClientRect();

            if (enemy.getAttribute("hp") !== "0") {
                const gridLength =
                    document.getElementById("valley").offsetWidth;
                ...
                if (
                    ...
                    enemy.style.left = `${
                        mobLocation.x + window.scrollX + step
                    }px`;
                } else if (
                    mobLocation.top + window.scrollY <
                    endPt.top + window.scrollY + 14 - gridLength * 3
                    ...
                    enemy.style.top = `${
                        mobLocation.y + window.scrollY + step
                    }px`;
                } else if (
                    mobLocation.left + window.scrollX >
                    ...
                    const castleDmgSound =
                        document.getElementById("castleDmgSound");
                    castleDmgSound.play();

                    let hp = document.getElementById("healthInt").innerText;
                    const currentHp = parseInt(hp);
                    if (currentHp > 0) {
                        document.getElementById("healthInt").innerText =
                            currentHp - 1;
                    } else {
        ...
        const intervalID = setInterval(() => {
            ...
            if (++x === repetitions) {
                ...
                const checkForWaveEnd = () => {
                    const enemiesLeft =
                        document.getElementsByClassName("mob").length;
                    console.log();
                    if (enemiesLeft === 0) {
                        ...
                        const waveEndSound =
                            document.getElementById("waveEndSound");
                        waveEndSound.play();
    ...
    if (document.getElementById("selection") === null) {
            ...
            if (allEnemyList.length === 0) {
                ...
                for (let enemy of allEnemyList) {
                    ...
                    const dx =
                        towerCentre.x +
                        towerRadius -
                        (enemyLocation.x + enemyRadius);
                    const dy =
                        towerCentre.y +
                        towerRadius -
                        (enemyLocation.y + enemyRadius);
                ...
                if (enemyList.length === 0) {
                    ...
                    const target = document.getElementById(
                        enemyList.sort((a, b) => (a.dist < b.dist ? a : b))[0]
                            .id
                    );
                    return target;
                }
        ...
        const shoot = () => {
            ...
            const damageMob = () => {
                ...
                if (newHp <= 0) {
                    ...
                    const mobDeathSound =
                        document.getElementById("mobDeathSound");
                    mobDeathSound.play();
                }
            ...
            const projectileMotion = () => {
                if (findTarget() !== null) {
                    ...
                    const mobRadius =
                        findTarget().children[2].getBoundingClientRect().width /
                        2;
                    const mobLocation = {
        ...
        const withinRange = () => {
            if (findTarget() !== null) {
                ...
                const dx =
                    towerCentre.x + towerRadius - (mobLocation.x + mobRadius);
                const dy =
                    towerCentre.y + towerRadius - (mobLocation.y + mobRadius);
                const dist = Math.sqrt(dx ** 2 + dy ** 2);
        ...
        const shoot = () => {
            ...
            const damageArea = () => {
                ...
                if (allEnemyList.length !== 0) {
                    ...
                    for (let enemy of allEnemyList) {
                        const enemyRect =
                            enemy.children[2].getBoundingClientRect();
                        const enemyRadius = enemyRect.width / 2;
